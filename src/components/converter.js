function Converter(color) {
  ...
  const toColorObject = c => {
    ...
    if (
      ...
      arr = c.split(',')
      if (typ == 'rgb') {
        if (arr.length != arrlength) return emptyObject()
  ...
  const color = toColorObject(color)
  ...
  const ncolToRgb = (ncol, white, black) => {
  ...
  const toHslString = () => {
    return 'hsl(' + hue + ', ' + Math.round(sat * 100) + '%, ' + Math.round(lightness * 100) + '%)'
  }
  const toHslStringDecimal = () => {
  ...
  const toCmykStringDecimal = () => {
    return 'cmyk(' + cyan + ', ' + magenta + ', ' + yellow + ', ' + black + ')'
  }
  const toNcolString = () => {
    return ncol + ', ' + Math.round(whiteness * 100) + '%, ' + Math.round(blackness * 100) + '%'
  }
  const toNcolStringDecimal = () => {
  ...
  const toHsl = () => {
    return { h: hue, s: sat, l: lightness, a: opacity }
  }
  const toHwb = () => {
    return { h: hue, w: whiteness, b: blackness, a: opacity }
  }
  const toCmyk = () => {
    return { c: cyan, m: magenta, y: yellow, k: black, a: opacity }
  }
  const toNcol = () => {
    return { ncol: ncol, w: whiteness, b: blackness, a: opacity }
  }
  const isDark = n => {
    let m = n || 128
    return (red * 299 + green * 587 + blue * 114) / 1000 < m
  }
  const saturate = n => {
    let x, rgb, color
    x = n / 100 || 0.1
    ...
    color = colorObject(rgb, opacity, hue, sat)
    attachValues(color)
  }
  const desaturate = n => {
    let x, rgb, color
